 ...
 *
 * @since 1.7.0
 * @param string $basic_or_pro basic|pro.
...
function wcpbc_update_product_pricing( $post_id, $zone, $data = array() ) {
	...
	if ( empty( $data['_price_method'] ) || wcpbc_is_exchange_rate( $data['_price_method'] ) ) {
		...
		if ( ! empty( $data['_sale_price'] ) && empty( $data['_sale_price_dates_to'] ) && empty( $data['_sale_price_dates_from'] ) ) {
			$data['_price'] = $data['_sale_price'];
		} elseif ( ! empty( $data['_sale_price'] ) && $data['_sale_price_dates_from'] && $data['_sale_price_dates_from'] <= strtotime( 'NOW', current_time( 'timestamp' ) ) ) {
			$data['_price'] = $data['_sale_price'];
		} else {
